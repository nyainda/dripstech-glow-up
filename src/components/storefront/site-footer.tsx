...
export function SiteFooter() {
  return <footer className="border-t border-border bg-brand-dark text-brand-light/75">
    <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 sm:grid-cols-2 sm:px-6 lg:grid-cols-5 lg:px-8">
      ...
      {groups.map((group) => <div key={group.title}><h2 className="text-sm font-bold text-brand-light">{group.title}</h2><ul className="mt-4 space-y-3 text-sm">{group.links.map(([label, to]) => <li key={to}><a href={to} className="hover:text-gold">{label}</a></li>)}</ul></div>)}
    </div>
