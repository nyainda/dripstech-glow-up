  ...
  return (
    <main className="min-h-screen bg-paper text-ink">
      ...
      <section id="catalogue" className="mx-auto max-w-7xl px-4 py-14 sm:px-6 lg:px-8">
        <div className="mb-8 flex flex-wrap items-end justify-between gap-4">
         ...
         {shownProducts.length > 0 ? <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">{shownProducts.map((product) => <article key={product.id} className="group flex flex-col overflow-hidden rounded-lg border border-ink/10 bg-surface">
           <Button type="button" variant="ghost" onClick={() => setSelectedProduct(product)} className="relative h-auto overflow-hidden rounded-none bg-paper p-0 text-left shadow-none" aria-label={`View ${product.name}`}><img src={product.images[0] ?? fittingsImage} alt={product.name} width={816} height={816} loading="lazy" className="aspect-square w-full object-contain p-4 transition-transform duration-500 group-hover:scale-[1.03]" /><span className="absolute left-2 top-2 rounded-full bg-ink/90 px-2 py-1 text-[10px] font-semibold uppercase text-paper">{product.in_stock ? "In stock" : "Check availability"}</span></Button>
      ...
      <Dialog open={selectedProduct !== null} onOpenChange={(open) => { if (!open) setSelectedProduct(null); }}>
        {selectedProduct && <DialogContent className="max-h-[90vh] w-[calc(100%-1rem)] max-w-3xl overflow-y-auto border-ink/15 bg-paper p-0 sm:rounded-lg">
          <div className="grid md:grid-cols-2">
            <div className="bg-surface p-4 md:p-6"><img src={selectedProduct.images[0] ?? fittingsImage} alt={selectedProduct.name} className="h-52 w-full object-contain md:aspect-square md:h-auto" /></div>
